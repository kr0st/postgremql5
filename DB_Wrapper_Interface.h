...
class DB_Wrapper_Interface
{
    public:
        ...
        virtual __int64 begin_transaction() = 0;
        virtual std::string commit(const __int64 trans_ptr) = 0;
        virtual std::string rollback(const __int64 trans_ptr) = 0;

        virtual std::string exec(const __int64 trans_ptr, const std::string& query) = 0;
